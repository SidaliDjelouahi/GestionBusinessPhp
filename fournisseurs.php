...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    try {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM achats WHERE id_fournisseur = ?");
        $stmt->execute([$id]);
        $count = (int)$stmt->fetchColumn();
        if ($count > 0) {
            $message = "Ce fournisseur a des achats associés, suppression impossible.";
            $message_type = "danger";
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nom === '') {
        ...
        try {
            if ($edit_id) {
                $stmt = $pdo->prepare("UPDATE fournisseurs SET nom = ?, adresse = ?, telephone = ? WHERE id = ?");
...
// ─── FETCH FOURNISSEURS WITH STATS ─────────────────────────────────────────
$search = trim($_GET['search'] ?? '');
$fournisseurs = [];
try {
    $sql = "SELECT
                f.id, f.nom, f.adresse, f.telephone,
                COUNT(DISTINCT a.id) AS nb_achats,
                COALESCE(SUM(ad.prix_achat * ad.qte), 0) AS total_achats,
                COALESCE(SUM(a.versement), 0) AS total_verse,
                COALESCE(SUM(ad.prix_achat * ad.qte), 0) - COALESCE(SUM(a.versement), 0) AS solde_du
            FROM fournisseurs f
            LEFT JOIN achats a ON a.id_fournisseur = f.id
            LEFT JOIN achat_details ad ON ad.id_achat = a.id";
    if ($search !== '') {
        $sql .= " WHERE f.nom LIKE ? OR f.telephone LIKE ?";
        $sql .= " GROUP BY f.id, f.nom, f.adresse, f.telephone ORDER BY f.id DESC";
...
// ─── COMPUTED STATS ────────────────────────────────────────────────────────
$stat_total_fournisseurs = count($fournisseurs);
$stat_total_achats       = array_sum(array_column($fournisseurs, 'total_achats'));
$stat_dettes             = array_sum(array_filter(array_column($fournisseurs, 'solde_du'), fn($v) => $v > 0));
...
<head>
    ...
    <title>Fournisseurs — G-Business</title>
...
<div id="main-wrapper">
    ...
    <header id="topbar">
        ...
        <div class="topbar-title">
            ...
            <small><?= count($fournisseurs) ?> fournisseur<?= count($fournisseurs) > 1 ? 's' : '' ?> en répertoire</small>
        </div>
    ...
    <main id="page-content">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div>
                <h1 style="font-size:22px;font-weight:800;color:var(--text-primary);margin:0;">
                    <i class="fa-solid fa-truck me-2" style="color:#6366f1;"></i>
                    Fournisseurs
                </h1>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-info">
                    <div class="stat-value"><?= $stat_total_fournisseurs ?></div>
                    <div class="stat-label">Total Fournisseurs</div>
                </div>
            </div>
            <div class="stat-card">
                <div class="stat-icon" style="background:var(--grad-purple);">
                    <i class="fa-solid fa-cart-shopping"></i>
                </div>
                <div class="stat-info">
                    <div class="stat-value"><?= number_format($stat_total_achats, 2, ',', ' ') ?> DA</div>
                    <div class="stat-label">Total Achats</div>
                </div>
            </div>
            <div class="stat-card">
                <div class="stat-icon" style="background:var(--grad-orange);">
                    <i class="fa-solid fa-hand-holding-dollar"></i>
                </div>
                <div class="stat-info">
                    <div class="stat-value"><?= number_format($stat_dettes, 2, ',', ' ') ?> DA</div>
        ...
        <div class="section-card" style="margin-bottom:24px;" id="formSection">
            <div class="section-card-header">
                <div class="section-card-title">
                    <i class="fa-solid fa-<?= $edit_fournisseur ? 'pen-to-square' : 'truck-ramp-box' ?>"></i>
                    <?= $edit_fournisseur ? 'Modifier le Fournisseur' : 'Ajouter un Fournisseur' ?>
                </div>
                <?php if ($edit_fournisseur): ?>
                    <a href="fournisseurs.php" class="btn-secondary-custom" style="padding:6px 14px;">
            ...
            <div class="section-card-body">
                <form method="POST" action="fournisseurs.php">
                    <?php if ($edit_fournisseur): ?>
                        <input type="hidden" name="edit_id" value="<?= (int)$edit_fournisseur['id'] ?>">
                    <?php endif; ?>
                    <div class="form-grid">
                        <div>
                            <label class="form-label-custom">Nom du fournisseur *</label>
                            <input type="text" name="nom" class="form-control-custom" placeholder="Ex: SARL Matériaux Plus"
                                   value="<?= htmlspecialchars($edit_fournisseur['nom'] ?? '') ?>" required>
                        </div>
                        <div>
                            ...
                            <input type="tel" name="telephone" class="form-control-custom" placeholder="Ex: 0555 12 34 56"
                                   value="<?= htmlspecialchars($edit_fournisseur['telephone'] ?? '') ?>">
                        </div>
                        <div>
                            <label class="form-label-custom">Adresse</label>
                            <textarea name="adresse" class="form-control-custom" rows="2" placeholder="Ex: Zone Industrielle, Oran"><?= htmlspecialchars($edit_fournisseur['adresse'] ?? '') ?></textarea>
                        </div>
                    </div>
                    <div style="display:flex;gap:10px;">
                        <button type="submit" class="btn-primary-custom">
                            <i class="fa-solid fa-<?= $edit_fournisseur ? 'save' : 'plus' ?>"></i>
                            <?= $edit_fournisseur ? 'Enregistrer' : 'Ajouter' ?>
                        </button>
                        <?php if ($edit_fournisseur): ?>
                            <a href="fournisseurs.php" class="btn-secondary-custom">Annuler</a>
                        <?php endif; ?>
                    </div>
                </form>
            </div>
        ...
        <div class="section-card">
            <div class="section-card-header">
                <div class="section-card-title">
                    ...
                    <span style="font-size:11px;font-weight:400;color:var(--text-muted);margin-left:4px;">
                        — <?= count($fournisseurs) ?> résultat<?= count($fournisseurs) > 1 ? 's' : '' ?>
                    </span>
                </div>
                <form method="GET" action="fournisseurs.php" style="margin:0;">
            ...
            <?php if (empty($fournisseurs)): ?>
                <div class="empty-state">
                    <i class="fa-solid fa-truck-fast d-block"></i>
                    <p>Aucun fournisseur trouvé<?= $search ? ' pour "' . htmlspecialchars($search) . '"' : '' ?></p>
                </div>
            <?php else: ?>
                    ...
                    <table class="table-custom">
                        ...
                        <tbody>
                            <?php foreach ($fournisseurs as $f):
                                $solde_du      = floatval($f['solde_du'] ?? 0);
                                $total_achats  = floatval($f['total_achats'] ?? 0);
                            ?>
                            <tr>
                                <td style="color:var(--text-muted);"><?= (int)$f['id'] ?></td>
                                <td class="td-main">
                                    <div style="display:flex;align-items:center;gap:10px;">
                                        <span style="width:32px;height:32px;border-radius:8px;background:linear-gradient(135deg,#f97316,#ea580c);color:#fff;font-size:12px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                                            <?= strtoupper(mb_substr($f['nom'], 0, 2)) ?>
                                        </span>
                                        <?= htmlspecialchars($f['nom']) ?>
                                    </div>
                                </td>
                                <td><?= htmlspecialchars($f['telephone'] ?? '—') ?></td>
                                <td style="max-width:180px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="<?= htmlspecialchars($f['adresse'] ?? '') ?>">
                                    <?= htmlspecialchars($f['adresse'] ?? '—') ?>
                                </td>
                                <td><?= (int)($f['nb_achats'] ?? 0) ?></td>
                                <td style="font-weight:600;color:<?= $total_achats > 0 ? '#8b5cf6' : 'var(--text-muted)' ?>;">
                                    <?= number_format($total_achats, 2, ',', ' ') ?> DA
                                </td>
                                <td>
                                    <?php if ($solde_du <= 0): ?>
                                        <span class="badge-stock badge-ok">Soldé</span>
                                    <?php else: ?>
                                        <span class="badge-stock badge-orange"><?= number_format($solde_du, 2, ',', ' ') ?> DA</span>
                                    <?php endif; ?>
                                </td>
                                <td style="text-align:center;">
                                    <div style="display:flex;gap:6px;justify-content:center;">
                                        <a href="fournisseurs.php?edit=<?= (int)$f['id'] ?>#formSection" class="btn-action edit" title="Modifier">
                                            <i class="fa-solid fa-pen"></i>
                                        </a>
                                        <a href="fournisseurs.php?delete=<?= (int)$f['id'] ?>" class="btn-action delete" title="Supprimer"

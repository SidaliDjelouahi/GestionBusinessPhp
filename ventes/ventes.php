...
// ─── FLASH MESSAGE ──────────────────────────────────────────────────────────
$msg_map = [
    'created' => ['Vente créée avec succès.',    'success'],
    'updated' => ['Vente modifiée avec succès.', 'success'],
    'deleted' => ['Vente supprimée avec succès.','success'],
...
if ($search !== '') {
    $where[]  = '(v.num LIKE ? OR c.nom LIKE ?)';
    $params[] = '%' . $search . '%';
    $params[] = '%' . $search . '%';
}
...
$sql = "
    SELECT
        ...
        COALESCE(SUM(vd.prix_vente * vd.qte), 0) AS montant_total,
        v.versement
    FROM ventes v
...
<head>
    ...
    <title>Historique des Ventes — G-Business</title>
    <meta name="description" content="Consultez et gérez l'historique complet de vos ventes clients.">
...
<div id="main-wrapper">
    ...
    <header id="topbar">
        ...
        <div class="topbar-title">
            ...
            <small><?= count($ventes) ?> vente<?= count($ventes) > 1 ? 's' : '' ?> trouvée<?= count($ventes) > 1 ? 's' : '' ?></small>
        </div>
    ...
    <main id="page-content">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4" style="flex-wrap:wrap;gap:12px;">
            <div>
                <h1 style="font-size:22px;font-weight:800;color:var(--text-primary);margin:0;">
                    <i class="fa-solid fa-clock-rotate-left me-2" style="color:#10b981;"></i>
                    Historique des Ventes
                </h1>
                <p style="font-size:13px;color:var(--text-muted);margin:4px 0 0;">
                    Consultez et gérez tous vos bons de vente clients
                </p>
            </div>
            <a href="bonVente.php" class="btn-primary-custom" id="btn-nouvelle-vente">
        ...
        <!-- SEARCH & FILTER BAR -->
        <form method="GET" action="ventes.php" id="filterForm">
            <div class="filter-bar">
                ...
                <div class="filter-group" style="align-self:flex-end;">
                    <a href="ventes.php" class="btn-secondary-custom" style="padding:9px 16px;" id="btn-show-all">
        ...
        <div class="section-card">
            <div class="section-card-header">
                <div class="section-card-title">
                    ...
                    <span style="font-size:11px;font-weight:400;color:var(--text-muted);margin-left:4px;">
                        — <?= count($ventes) ?> résultat<?= count($ventes) > 1 ? 's' : '' ?>
                    </span>
                </div>
            </div>
            ...
            <?php if (empty($ventes)): ?>
                <div class="empty-state">
                    ...
                    <p>Aucune vente trouvée<?= ($date_from || $date_to || $search) ? ' pour ce filtre' : '' ?>.</p>
                    <?php if ($date_from || $date_to || $search): ?>
                        <a href="ventes.php" style="color:var(--accent);font-size:13px;margin-top:8px;display:inline-block;">
                            Effacer les filtres
                        </a>
                    <?php endif; ?>
                </div>
            <?php else: ?>
                <div style="overflow-x:auto;">
                    <table class="table-custom" id="ventesTable">
                        <thead>
                            <tr>
                                <th>N° BL</th>
                                <th>Client</th>
                        ...
                        <tbody>
                            <?php foreach ($ventes as $v):
                                ...
                                $versement = floatval($v['versement']);
                                ...
                                $date_fmt = date('d/m/Y H:i', strtotime($v['date']));
                            ?>
                            <tr>
                                <td class="td-main"><?= htmlspecialchars($v['num']) ?></td>
                                <td><?= $v['client'] ? htmlspecialchars($v['client']) : '<span style="color:var(--text-muted);">—</span>' ?></td>
                                <td style="white-space:nowrap;color:var(--text-secondary);"><?= $date_fmt ?></td>
                                <td style="text-align:center;">
                                    <span class="badge-pill-blue"><?= (int)$v['nb_articles'] ?> art.</span>
                                </td>
                                <td style="text-align:center;">
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content" style="border-radius:14px;border:none;overflow:hidden;">
            <div class="modal-detail-header d-flex align-items-center justify-content-between">
                <div>
                    <div class="modal-title" id="detailModalLabel">Détails Vente</div>
...
<form id="deleteForm" method="POST" action="_delete.php" style="display:none;">
    <input type="hidden" name="id" id="deleteVenteId">
</form>
...
document.querySelectorAll('.btn-details').forEach(btn => {
    btn.addEventListener('click', () => {
        currentVenteId = btn.dataset.id;
        openDetailModal(currentVenteId);
    });
});
...
function openDetailModal(id) {
    ...
    modalLabel.textContent = 'Détails Vente';
    modalSub.textContent   = '';
    detailModal.show();

    fetch('_get_vente.php?id=' + encodeURIComponent(id))
        .then(r => r.json())
        .then(data => {
...
function renderModalContent(data) {
    ...
    const versement = parseFloat(v.versement || 0);
    const reste     = total - versement;

    modalLabel.textContent = 'Détails Vente N° ' + v.num;
    modalSub.textContent   = 'Client: ' + (v.client_nom || '—');
    ...
    const date_fmt = v.date ? new Date(v.date).toLocaleString('fr-FR') : '—';
    ...
    details.forEach(d => {
        rows += `<tr>
            ...
            <td style="text-align:right;">${parseFloat(d.prix_vente).toLocaleString('fr-FR', {minimumFractionDigits:2,maximumFractionDigits:2})} DA</td>
    ...
    modalContent.innerHTML = `
        <div class="modal-info-grid">
            <div class="modal-info-item">
                ...
                <div class="modal-info-value">${escHtml(v.client_nom || '—')}</div>
            </div>
            <div class="modal-info-item">
    ...
    btnModifier.onclick  = () => { window.location.href = 'bonVente.php?id=' + v.id; };
    btnSupprimer.onclick = () => {
        if (confirm('Êtes-vous sûr de vouloir supprimer cette vente ? Cette action est irréversible.')) {
            document.getElementById('deleteVenteId').value = v.id;

...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    try {
        ...
        $edit_vente = $stmt->fetch(PDO::FETCH_ASSOC);
...
try {
    $today = date('Y-m-d');
    $stmt  = $pdo->prepare("SELECT COUNT(*) FROM ventes WHERE DATE(date) = ?");
    $stmt->execute([$today]);
    $n = (int)$stmt->fetchColumn();
} catch (Exception $e) { $n = 0; }
$auto_num = 'VTE-' . date('Ymd') . '-' . str_pad($n + 1, 3, '0', STR_PAD_LEFT);
...
} catch (Exception $e) { $all_clients = []; }

// ─── ERROR / SUCCESS FROM SAVE ──────────────────────────────────────────────
$save_error   = '';
$save_success = '';
if (isset($_GET['error'])) {
...
if (isset($_GET['success'])) {
    $save_success = trim(urldecode($_GET['success']));
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <meta name="description" content="Créez ou modifiez un bon de vente client.">
...
<aside id="sidebar" aria-label="Navigation principale">
    ...
    <nav class="sidebar-nav">
        ...
        <div class="nav-item">
            <a href="ventes.php" class="nav-link-item active" data-label="Ventes" id="nav-ventes">
...
<div id="main-wrapper">
    <header id="topbar">
        ...
        <div class="topbar-title">
            ...
            <small><?= $edit_vente ? 'N° ' . htmlspecialchars($edit_vente['num']) : 'N° ' . htmlspecialchars($auto_num) ?></small>
        </div>
    ...
    <main id="page-content">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4" style="flex-wrap:wrap;gap:12px;">
            <div>
                <h1 style="font-size:22px;font-weight:800;color:var(--text-primary);margin:0;">
                    <i class="fa-solid fa-<?= $edit_vente ? 'pen-to-square' : 'cash-register' ?> me-2" style="color:#10b981;"></i>
                    <?= htmlspecialchars($page_title) ?>
                </h1>
                <p style="font-size:13px;color:var(--text-muted);margin:4px 0 0;">
                    <?= $edit_vente ? 'Modifiez les détails de cette vente' : 'Créez un nouveau bon de vente client' ?>
                </p>
            </div>
            <a href="ventes.php" class="btn-secondary-custom" id="btn-back-list">
        ...
        <!-- MAIN FORM -->
        <form method="POST" action="_save.php" id="bonVenteForm" novalidate>
            <?php if ($edit_id): ?>
                <input type="hidden" name="edit_id" value="<?= (int)$edit_id ?>">
            <?php endif; ?>
            <input type="hidden" name="num"        id="hiddenNum"  value="<?= htmlspecialchars($edit_vente['num'] ?? $auto_num) ?>">
            <input type="hidden" name="date_vente" id="hiddenDate" value="<?= date('Y-m-d H:i:s') ?>">
                    ...
                    <div class="section-card search-card" style="margin-bottom:20px;">
                        ...
                        <div>
                            <div class="search-controls">
                                ...
                                <div>
                                    <label class="form-label-custom">P.Vente (DA)</label>
                ...
                <div class="pos-right">
                    ...
                    <div class="section-card" style="margin-bottom:16px;">
                        <div style="padding:16px;">
                            <label class="form-label-custom" for="selectClient">Client <span style="font-size:10px;color:var(--text-muted);">(optionnel)</span></label>
                            <select name="id_clients" id="selectClient" class="form-control-custom">
                                <option value="">-- Sélectionner un client --</option>
                                <?php foreach ($all_clients as $c): ?>
                                    <option value="<?= (int)$c['id'] ?>"
                                        <?= ($edit_vente && $edit_vente['id_clients'] == $c['id']) ? 'selected' : '' ?>>
                    ...
                    <div class="ticket-panel">
                        <div class="ticket-header">
                            ...
                            <div class="ticket-sub">Bon de Vente</div>
                            <div class="ticket-num">N° <?= htmlspecialchars($edit_vente['num'] ?? $auto_num) ?></div>
                        </div>
                        ...
                        <div style="border-top: 1px dashed var(--border-color); padding-top:12px; margin-top:4px;">
                            <label class="form-label-custom" for="versementInput">Montant reçu (DA)</label>
                            <input type="number" name="versement" id="versementInput" class="form-control-custom"
                                   value="<?= htmlspecialchars($edit_vente['versement'] ?? '0') ?>"
                        ...
                        <button type="submit" class="btn-valider" id="btnValiderVente">
                            <i class="fa-solid fa-check-circle"></i>
                            <?= $edit_vente ? 'Mettre à jour' : "Valider la Vente" ?>
                        </button>
                        <a href="ventes.php" class="btn-secondary-custom" style="width:100%;justify-content:center;margin-top:10px;" id="btn-annuler">
...
function renderDrop(items) {
    ...
    prodDropdown.innerHTML = items.map((p, i) => {
        ...
        return `<div class="prod-drop-item ${cls}" data-idx="${i}" title="Clic: sélectionner | Entrée: ajouter directement">
            ...
            <span class="prod-drop-meta">
                ${fmtNum(p.prix_vente)} DA&nbsp;&nbsp;|&nbsp;&nbsp;achat: ${fmtNum(p.prix_achat)} DA&nbsp;&nbsp;|&nbsp;&nbsp;${fmtQte(qte)} u.
            </span>
    ...
    prodDropdown.querySelectorAll('.prod-drop-item').forEach(el => {
        el.addEventListener('mousedown', (e) => {
            ...
            prixInput.value = parseFloat(item.prix_vente).toFixed(2);
...
function navigateDrop(dir) {
    ...
    els[highlightedIdx].scrollIntoView({ block: 'nearest' });
    if (dropdownItems[highlightedIdx]) {
        prixInput.value = dropdownItems[highlightedIdx].prix_vente;
    }
}
...
function enterHighlighted() {
    ...
    if (selectedProduct) {
        addProductToTable(
            selectedProduct.id,
            selectedProduct.nom,
            prixInput.value || selectedProduct.prix_vente,
            qtyInput.value || 1
        );
        selectedProduct = null;
    }
...
/* ── FORM VALIDATION ── */
document.getElementById('bonVenteForm').addEventListener('submit', function(e) {
    ...
    const rows = document.getElementById('detailLines').querySelectorAll('tr');
    const clientValeur = document.getElementById('selectClient').value;
    const versementValeur = parseFloat(document.getElementById('versementInput').value || 0);

    if (rows.length === 0 && (clientValeur === '' || versementValeur <= 0)) {
        e.preventDefault();
        errSearch.textContent = 'Pour valider un bon sans produits, vous devez obligatoirement sélectionner un client ET saisir un versement.';
...
function openAddClientModal() {
    ...
    document.getElementById('modalClientError').style.display = 'none';
    document.getElementById('modalClientError').textContent   = '';

    const modal = document.getElementById('modalClient');
    modal.style.display = 'flex';

    setTimeout(() => document.getElementById('mc_nom').focus(), 80);
}
...
document.getElementById('modalClient').addEventListener('click', function(e) {
...
document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') {
        const modal = document.getElementById('modalClient');
        if (modal.style.display === 'flex') closeAddClientModal();
    }
});

['mc_nom', 'mc_telephone'].forEach(id => {
...
function saveClient() {
    ...
    const telephone = document.getElementById('mc_telephone').value.trim();
    const adresse   = document.getElementById('mc_adresse').value.trim();
    const errEl     = document.getElementById('modalClientError');
    const btn       = document.getElementById('btnSaveClient');
    const spinner   = document.getElementById('btnSaveClientSpinner');
    const icon      = document.getElementById('btnSaveClientIcon');

    if (nom === '') {
        errEl.textContent   = 'Le nom du client est obligatoire.';
    ...
    .then(data => {
        ...
        data.all_clients.forEach(c => {
            const opt = document.createElement('option');
            opt.value       = c.id;
            opt.textContent = c.nom;
            if (c.id === data.new_id) opt.selected = true;
        ...
        confirmMsg.textContent   = '✅ Client "' + escHtml(data.new_nom) + '" ajouté et sélectionné !';
...
/* ── SUCCESS ALERT: auto-dismiss + reset form after 2s ── */
(function() {
    const successAlert = document.getElementById('successAlert');
    if (!successAlert) return;

    setTimeout(() => {
        // 1. Fade out the alert smoothly
        successAlert.style.transition = 'opacity 0.4s ease';
        successAlert.style.opacity = '0';
        setTimeout(() => successAlert.remove(), 400);

        // 2. Reset the form to a blank new vente
        //    a. Clear all product rows
        document.getElementById('detailLines').innerHTML = '';

        //    b. Reset versement to 0 and clear manualEdit flag
        const versementEl = document.getElementById('versementInput');
        versementEl.value = '0';
        delete versementEl.dataset.manualEdit;

        //    c. Reset client select to default
        const clientSelect = document.getElementById('selectClient');
        if (clientSelect) clientSelect.value = '';

        //    d. Reset search inputs
        const prodSearch = document.getElementById('prodSearch');
        const qtyInput   = document.getElementById('qtyInput');
        const prixInput  = document.getElementById('prixInput');
        ...
        //    e. Generate new auto-num for the hidden input
        ...
        if (hiddenNum) {
            ...
            const current = hiddenNum.value || '';
            const match   = current.match(/(\d+)$/);
            const nextNum = match ? (parseInt(match[1]) + 1) : 1;
            hiddenNum.value = 'VTE-' + ymd + '-' + String(nextNum).padStart(3,'0');
        }

        //    f. Remove edit_id hidden input if present (ensure new mode)
        const editIdInput = document.querySelector('input[name="edit_id"]');
        if (editIdInput) editIdInput.remove();

        //    g. Recalculate totals (resets ticket panel to 0)
        if (typeof calcTotals === 'function') calcTotals();

        //    h. Remove ?success= from URL without page reload
        const url = new URL(window.location.href);
        url.searchParams.delete('success');
        window.history.replaceState({}, '', url.toString());

        //    i. Focus on product search bar ready for next entry
        if (prodSearch) setTimeout(() => prodSearch.focus(), 50);
...
<div id="modalClient" style="
    ...
    <div style="
        ...
        <!-- Close button -->
        <button type="button" onclick="closeAddClientModal()" style="
        ...
        <div style="font-size:16px;font-weight:800;color:var(--text-primary);margin-bottom:4px;">
            <i class="fa-solid fa-user-plus" style="color:#10b981;margin-right:8px;"></i>
            Nouveau Client
        </div>
        <div style="font-size:12px;color:var(--text-muted);margin-bottom:20px;">
            Le client sera ajouté et sélectionné automatiquement.
        </div>

        <!-- Error message -->
        <div id="modalClientError" style="
        ...
        <div style="display:flex;flex-direction:column;gap:14px;">
            ...
            <div>
                <label class="form-label-custom" for="mc_nom">
                    Nom du client *
                </label>
                <input type="text" id="mc_nom" class="form-control-custom"
                       placeholder="Ex: Ahmed Bouzid"
                       autocomplete="off">
            </div>

            <div>
                <label class="form-label-custom" for="mc_telephone">
                    Téléphone
                </label>
                <input type="tel" id="mc_telephone" class="form-control-custom"
                       placeholder="Ex: 0555 123 456">
            </div>

            <div>
                <label class="form-label-custom" for="mc_adresse">
                    Adresse
                </label>
                <textarea id="mc_adresse" class="form-control-custom"
        ...
        <div style="display:flex;gap:10px;margin-top:20px;">
            <button type="button" id="btnSaveClient"
                ...
                <i class="fa-solid fa-check" id="btnSaveClientIcon"></i>
                Ajouter
            </button>
